  ...
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t idx = getLIdx(x,y,ch,width,height);
        arr1[idx] += arr2[idx];
      }
}
//----------------------------------------------------------------------------
// Explicit instantiations:
template void addToArray(float *arr1,
int width, int height, int channels,
const float *arr2);
...
//----------------------------------------------------------------------------
// Explicit instantiations:
template void subFromArray(float *arr1,
int width, int height, int channels,
const float *arr2);
                         ...
                         T2 * matlab)
{
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t lin_matlab = getLIdxMatlab(x,y,ch,width,height);
        size_t lin_data = getLIdx(x,y,ch,width,height);
        matlab[lin_matlab] = (T2) data[lin_data];
      }
}//############################################################################
// Explicit instantiations:
template void alignDataCPU2Matlab(const float *data,
size_t width, size_t height, size_t channels,
float *matlab);
template void alignDataCPU2Matlab(const bool *data,
size_t width, size_t height, size_t channels,
bool *matlab);
template void alignDataCPU2Matlab(const float *data,
size_t width, size_t height, size_t channels,
...
void alignDataCPU2OpCV(const T1 *data,
                       size_t width, size_t height, size_t channels,
                       T2 *opcv,
                       size_t step_y, size_t step_x) {
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t lin_opcv = getLIdxOpenCV(x,y,ch,step_x,step_y);
        size_t lin_data = getLIdx(x,y,ch,width,height);
        opcv[lin_opcv] = (T1) data[lin_data];
      }
}//############################################################################
// Explicit instantiations:
template void alignDataCPU2OpCV(const float *data,
size_t  width, size_t  height, size_t  channels,
float *opcv,
size_t  step_y, size_t  step_x);
template void alignDataCPU2OpCV(const bool *data,
size_t  width, size_t  height, size_t  channels,
bool *opcv,
size_t  step_y, size_t  step_x);
template void alignDataCPU2OpCV(const bool *data,
  ...
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t lin_matlab = getLIdxMatlab(x,y,ch,width,height);
        size_t lin_data = getLIdx(x,y,ch,width,height);
        data[lin_data] = (T1) matlab[lin_matlab];
      }

}//############################################################################
// Explicit instantiations:
template void alignDataMatlab2CPU(float *data,
size_t  width, size_t  height, size_t  channels,
const float *matlab );
template void alignDataMatlab2CPU(bool *data,
size_t  width, size_t  height, size_t  channels,
const bool *matlab );
  ...
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t lin_opcv = getLIdxOpenCV(x,y,ch,step_x,step_y);
        size_t lin_data = getLIdx(x,y,ch,width,height);
        data[lin_data] = (T1) opcv[lin_opcv];
      }
}
//############################################################################
// Explicit instantiations:
template void alignDataOpCV2CPU(float *data,
size_t width, size_t height, size_t channels,
const float *opcv,
size_t step_y, size_t  step_x);
template void alignDataOpCV2CPU(bool *data,
size_t width, size_t height, size_t channels,
const bool *opcv,
size_t step_y, size_t  step_x);
template void alignDataOpCV2CPU(bool *data,
...
//############################################################################
template<typename T>
void  divArray(T *num, const T *denom, int width, int height, int channels) {
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t idx = getLIdx(x,y,ch,width,height);
...
//----------------------------------------------------------------------------
// Explicit instantiations:
template void divArray<float>(float *num, const float *denom,
...
//############################################################################
template<typename T>
void divArray(T num, T * denom, int width, int height, int channels ) {
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t idx = getLIdx(x,y,ch,width,height);
...
//----------------------------------------------------------------------------
// Explicit instantiations:
template void divArray<float>(float num, float * denom,
int width, int height, int depth);
...
//############################################################################
template<typename T>
void mulArray(T *arr, int width, int height, int channels, T value ) {
  for (size_t ch=0; ch<channels; ch++)
    for (size_t y=0; y<height; y++)
      for (size_t x=0; x<width; x++) {
        size_t idx = ch*width*height + y*width + x;
        arr[idx] *= value;
      }
}
//----------------------------------------------------------------------------
// Explicit instantiations:
template void mulArray(float *arr,
int width, int height, int depth, float value);
template void mulArray(unsigned char *arr,
int width, int height, int depth, unsigned char value);
template void mulArray(int *arr,
int width, int height, int depth, int value);
template void mulArray(uint64_t *arr,
int width, int height, int depth, uint64_t value);
